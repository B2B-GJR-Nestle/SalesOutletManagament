...
def generate_scheduling(df, office_coord):
    # Sort dataframe by 'NAMA SALESMAN', 'DAY', 'KUNJUNGAN', and 'NAMA TOKO' columns
    df = df.sort_values(by=['NAMA SALESMAN', 'DAY', 'KUNJUNGAN', 'NAMA TOKO'])
    ...
    for salesman, group in df.groupby(['NAMA SALESMAN', 'DAY', 'KUNJUNGAN']):
    ...
    for salesman, days_data in visit_orders.items():
        for day, visit_orders in days_data.items():
            for visit_order, data in visit_orders.items():
                scheduling_data.append([salesman[0], day, salesman[1], visit_order, data['NAMA TOKO'], data['Distance'], data['Coordinates']])

    scheduling_df = pd.DataFrame(scheduling_data, columns=['NAMA SALESMAN', 'DAY', 'KUNJUNGAN', 'Visit Order', 'NAMA TOKO', 'Distance', 'Coordinates'])
...
def filter_schedule(scheduling_df, salesman, kunjungan):
    return scheduling_df[(scheduling_df['NAMA SALESMAN'] == salesman) & (scheduling_df['KUNJUNGAN'] == kunjungan)].copy()
...
if uploaded_file is not None:
    ...
    try:
        ...
        # Filter scheduling
        filtered_schedule = filter_schedule(scheduling_df, selected_salesman, selected_visit_type)
        ...
        if not filtered_schedule.empty:
            st.markdown(f'<span style="font-size:16px;">📍 Map showing connections for {selected_salesman} on {selected_day} that need to visit {filtered_schedule["Visit Order"].count()} outlet(s) around <b>{round(filtered_schedule["Distance"].sum(), 3)} km<b></span>', unsafe_allow_html=True)
            office_latitude = -6.558031
            office_longitude = 106.691809
