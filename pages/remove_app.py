...
def main():
    ...
    if not df.empty:
        ...
        # Step 2: Allow users to delete outlet names
        st.sidebar.markdown("## Delete Outlet")
        outlet_to_delete = st.sidebar.selectbox("Select Outlet to Delete", sorted(df['Outlet'].unique()), key="outlet_dropdown")

        if st.sidebar.button("Delete Outlet"):
            df = df[df['Outlet'] != outlet_to_delete].reset_index()

            # Re-cluster the outlets and reassign salesmen to centroids based on the updated dataframe
            kmeans_model = KMeans(n_clusters=len(unique_salesmen), random_state=42).fit(df[['Latitude', 'Longitude']])
            combined_kmeans_labels = kmeans_model.predict(df[['Latitude', 'Longitude']])

            # Recalculate centroids based on the updated dataframe
            updated_centroids = kmeans_model.cluster_centers_

            # Display combined dataframe
            st.sidebar.markdown("## Outlets Data")
            st.sidebar.write(df)

            # Update the map with the new dataframe
            st.markdown(f"## Updated Salesman Coverage Map")
            st.components.v1.html(plot_map(df, updated_centroids, combined_kmeans_labels, initial_centroid_salesman, salesman_mapping, bekasi_geojson_path, unique_salesmen), width=700, height=500)
